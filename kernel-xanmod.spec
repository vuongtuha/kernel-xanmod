...
%global certs %( for f in %_sourcedir/*.crt; do                                                         \
    ...
    cat "$f" >>%_sourcedir/certs/signing_key.pem                                                      \
    mkdir -p %_sourcedir/certs/signing_key.pem                                                          \
    openssl x509 -inform PEM -in "$f" -outform DER -out %_sourcedir/certs/signing_key.pem   \
    echo -n "$cert" ""                                                                                  \
done )
...
Name:           kernel-xanmod
Summary:        The Xanmod Kernel
License:        GPL-2.0-only
Group:          System/Kernel
Version:        6.0.12.Xmd.main
%if 0%{?is_kotd}
...
if [ %CONFIG_MODULE_SIG = "y" ]; then
	if [ -n "%certs" ] ; then
		ln -s %_sourcedir/certs/signing_key.pem .
	else
...
%endif
    ...
    tar --exclude=\*.ipa-clones --exclude=.config.old --exclude=.kernel-binary.spec.buildenv \
        --exclude=certs/signing_key.pem --exclude=.kernel.genkey \
